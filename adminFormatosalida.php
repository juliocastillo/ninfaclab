...
class Model_form {
    ...
    function get_field_id($id){
        ...
        $sql = "SELECT
                    id,
                    nombre
                FROM ctl_formatosalida WHERE id='$id'";
    ...
    function set_form($id) {
        ...
        $sql = "UPDATE ctl_formatosalida SET 
                   nombre ='$nombre'
                WHERE id='$id'";
        $db->consulta($sql);
    }
    function insert_form() {
        $db = new MySQL();
        $nombre = strtoupper($_POST['nombre']);
        $sql = "INSERT INTO ctl_formatosalida(
                    nombre) 
                VALUE (
                    '$nombre')";
        $db->consulta($sql);
    ...
    function make_table($consulta){
        ...
        $tblbody .= "<tr>".
            "<td><a href='adminFormatosalida.php?req=3&id=".$row['id']."'>".$row['nombre']."</td>".  
            "</tr>";
        }
        return $tblbody;
    }
...
if (!$req) {//ingresar nuevo registro desde cero
    ...
    $args = array ( // parametro que se pasaran a la vista
            'form' => 'adminFormatosalida.html',
            'FileName' => 'adminFormatosalida.php?req=2',
            'FormTitle' => 'Creación/Edición Formato salida del reporte',
            'id'=> '',
            'nombre' => '',
            'tblbody' => $tblbody
            );
...
elseif ($req == 2) {//ingresar un nuevo registro
    ...
    print "<script>window.location = 'adminFormatosalida.php'</script>";    
}
elseif ($req == 3) {//mostrar para modificar registro
    ...
    $args = array ( // parametro que se pasaran a la vista
            'form' => 'adminFormatosalida.html',
            'FileName' => 'adminFormatosalida.php?req=4',
