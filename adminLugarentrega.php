...
class Model_form {
    ...
    function get_field_id($id){
        ...
        $sql = "SELECT
                    id,
                    nombre
                FROM ctl_lugarentrega WHERE id='$id'";
    ...
    function set_form($id) {
        ...
        $sql = "UPDATE ctl_lugarentrega SET 
                   nombre ='$nombre'
                WHERE id='$id'";
        $db->consulta($sql);
    }
    function insert_form() {
        $db = new MySQL();
        $nombre = strtoupper($_POST['nombre']);
        $sql = "INSERT INTO ctl_lugarentrega(
                    nombre) 
                VALUE (
                    '$nombre')";
        $db->consulta($sql);
    ...
    function make_table($consulta){
        ...
        $tblbody .= "<tr>".
            "<td><a href='adminLugarentrega.php?req=3&id=".$row['id']."'>".$row['nombre']."</td>".  
            "</tr>";
        }
        return $tblbody;
    }
...
if (!$req) {//ingresar nuevo registro desde cero
    ...
    $args = array ( // parametro que se pasaran a la vista
            'form' => 'adminLugarentrega.html',
            'FileName' => 'adminLugarentrega.php?req=2',
            'FormTitle' => 'Creación/Edición Lugar entrega de resultados',
            'id'=> '',
            'nombre' => '',
            'tblbody' => $tblbody
            );
...
elseif ($req == 2) {//ingresar un nuevo registro
    ...
    print "<script>window.location = 'adminLugarentrega.php'</script>";    
}
elseif ($req == 3) {//mostrar para modificar registro
    ...
    $args = array ( // parametro que se pasaran a la vista
            'form' => 'adminLugarentrega.html',
            'FileName' => 'adminLugarentrega.php?req=4',
            'FormTitle' => 'Creación/Edición Lugar entrega de resultados',
            'id'=> $rec['id'],
            'nombre' => $rec['nombre'],
            'tblbody' => $tblbody
