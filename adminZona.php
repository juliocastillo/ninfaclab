...
class Model_form {
    ...
    function make_table($consulta){
        ...
        $tblbody .= "<tr>".
            "<td><a href='adminZona.php?req=3&id=".$row['id']."'>".$row['nombre']."</td>".  
            "</tr>";
        }
        return $tblbody;
    }
...
if (!$req) {//ingresar nuevo registro desde cero
    ...
    $args = array ( // parametro que se pasaran a la vista
            'form' => 'adminZona.html',
            'FileName' => 'adminZona.php?req=2',
            'FormTitle' => 'Creación/Edición Zonas del pais',
            'id'=> '',
            'nombre' => '',
            'tblbody' => $tblbody
            );
...
elseif ($req == 2) {//ingresar un nuevo registro
    ...
    print "<script>window.location = 'adminZona.php'</script>";    
}
